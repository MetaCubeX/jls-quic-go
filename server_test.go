...
func TestServerVersionNegotiation(t *testing.T) {
	...
	// JLS BEGIN: unsupported versions are classified by the camouflage upstream.
	t.Run("JLS forwards unsupported version", func(t *testing.T) {
		conn := newUDPConnLocalhost(t)
		var dialCount atomic.Int32
		var eventRecorder events.Recorder
		server := newTestServer(t, &serverOpts{
			eventRecorder:             &eventRecorder,
			disableVersionNegotiation: true,
			config: &Config{
		...
		require.Eventually(t, func() bool { return dialCount.Load() == 1 }, time.Second, 10*time.Millisecond)
	})
	// JLS END
}

// JLS BEGIN: pre-connection rejects use bounded camouflage forwarding.

func TestJLSServerForwardsPreConnectionRejects(t *testing.T) {
	for _, test := range []struct {
		name                  string
		packet                func(*testing.T, net.Addr) receivedPacket
		exhaustAuthentication bool
	}{
		{
			name: "short DCID",
			packet: func(t *testing.T, addr net.Addr) receivedPacket {
				return getValidInitialPacket(t, addr, randConnID(5), randConnID(7))
			},
		},
		{
			name: "greased Fixed Bit",
			packet: func(t *testing.T, addr net.Addr) receivedPacket {
				packet := getValidInitialPacket(t, addr, randConnID(8), randConnID(7))
				packet.data[0] &^= 0x40
				return packet
			},
		},
		{
			name: "authentication capacity exhausted",
			packet: func(t *testing.T, addr net.Addr) receivedPacket {
				return getValidInitialPacket(t, addr, randConnID(8), randConnID(7))
			},
			exhaustAuthentication: true,
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.exhaustAuthentication {
				server.jlsForwarder.mu.Lock()
				server.jlsForwarder.authenticationCount = jlsMaxAuthenticatingConns
				server.jlsForwarder.mu.Unlock()
			}

			conn := newUDPConnLocalhost(t)
			packet := test.packet(t, conn.LocalAddr())
			want := append([]byte(nil), packet.data...)
			server.handlePacket(packet)
...
func getJLSClientHello(t testing.TB, config *tls.Config) []byte {
	...
	require.NoError(t, checkClientHello(ev.Data))
	return ev.Data
}
...
func TestJLSServerForwardsPreAuthenticationTransportError(t *testing.T) {
	upstream, err := net.ListenPacket("udp", "127.0.0.1:0")
	require.NoError(t, err)
	defer upstream.Close()
	server := newTestServer(t, &serverOpts{
	...
	defer client.Close()
	payload, err := (&wire.StreamFrame{StreamID: 0, Data: []byte("probe")}).Append(nil, protocol.Version1)
	require.NoError(t, err)
	packet := composeJLSInitialPacketPayload(
		t,
		protocol.Version1,
		[]byte{1, 2, 3, 4, 5, 6, 7, 8},
		nil,
	...
	packet = append(packet, make([]byte, protocol.MinInitialPacketSize-len(packet))...)
	received := newJLSReceivedPacket(packet, client.LocalAddr())
	received.rcvTime = monotime.Now()
	server.handlePacket(received)

	require.NoError(t, upstream.SetReadDeadline(time.Now().Add(time.Second)))
	forwarded := make([]byte, protocol.MaxPacketBufferSize)
	n, _, err := upstream.ReadFrom(forwarded)
	require.NoError(t, err)
	require.Equal(t, packet, forwarded[:n])

	require.NoError(t, client.SetReadDeadline(time.Now().Add(50*time.Millisecond)))
	_, _, err = client.ReadFrom(make([]byte, protocol.MaxPacketBufferSize))
	var netErr net.Error
	require.ErrorAs(t, err, &netErr)
	require.True(t, netErr.Timeout(), "local CONNECTION_CLOSE reached the client: %v", err)
}

func TestJLSServerForwardsUnsupportedVersions(t *testing.T) {
	upstream, err := net.ListenPacket("udp", "127.0.0.1:0")
	require.NoError(t, err)
	defer upstream.Close()
	...
	const (
		...
		unadvertisedVersion = protocol.Version(0x12345678)
	)
	server := newTestServer(t, &serverOpts{
		newConn: newConnection,
		config: &Config{
	...
	for _, test := range []struct {
		...
		{name: "Reserved", version: reservedVersion, size: protocol.MinUnknownVersionPacketSize},
		{name: "UnadvertisedSmallPacket", version: unadvertisedVersion, size: protocol.MinUnknownVersionPacketSize - 1},
	} {
		t.Run(test.name, func(t *testing.T) {
			clientConn := newUDPConnLocalhost(t)
			defer clientConn.Close()
			remote := clientConn.LocalAddr()
			packet := make([]byte, test.size)
			packet[0] = 0xc0
			binary.BigEndian.PutUint32(packet[1:5], uint32(test.version))
			packet[5] = 8
			...
			n, forwardAddr, err := upstream.ReadFrom(got)
			require.NoError(t, err, "version %#x", test.version)
			require.Equal(t, packet, got[:n], "version %#x", test.version)
			_, err = upstream.WriteTo([]byte("response"), forwardAddr)
			require.NoError(t, err)
		})
	}
}
